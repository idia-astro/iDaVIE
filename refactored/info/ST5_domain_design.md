...
| **Active type** | The feature-set type the user is currently working with, set by tab click on the source-list panel or by the `DisplayNextSet` / `DisplayPreviousSet` voice command. Nullable — `null` means no source-list panel is open. Produced by ST5's source-list controller and consumed by ST5's `SelectionService`. |
...
- `WorldCoord` is **ST5-internal**: returned by `ICoordinateTransformer.Transform` (consumed from ST2), held by `VoTableSaver` and the spectral-profile display path. It does not appear on any provided interface.
...
- `FeatureSetService` — implements `IFeatureSetQuery` (cross-team — query, display mutation, set-membership mutation, event), `IFeatureSetCatalog` (ST5-internal — `CreateSet` factory method held by `FeatureImportService`), and `IFeatureStateCapture` (cross-team — persistence capture/restore per M-16; DTO field schema in draft per IR-01, canonical shape in `shared_interfaces.md` §5.6 and reproduced in `ST5_interface.md` §3). Owns the set store and the Mask creation flow (subscribes to `ISourceStatsProvider.SourceStatsUpdated`; on the first event lazily runs the §6.3 sequence using its own `IVisualiserBinder` and `IFeatureFactory` references). Assigns each set a monotonic `Index` (see `IFeatureSet.Index` in `ST5_interface.md` §3). Holds `IVolumeDataSet` (M-02) to reset the catalogue on `DatasetUnloaded` and to validate cube state for incoming mutation calls. For `IFeatureStateCapture`, only UserDefined / Imported / SelectionBox sets are snapshotted — Mask sets re-derive from ST2 on restore via `IMaskStateCapture` + the `SourceStatsUpdated` bootstrap. On `Restore`, `FeatureSetEntryDto.Type` is parsed via `InteractionStateDto.TryParseOrDefault` with a `FeatureSetType.UserDefined` fallback so that unknown future enum members degrade gracefully (see `shared_interfaces.md` §4.4).
...
- `FeatureFactory` — implements the ST5-internal `IFeatureFactory`; populates a caller-supplied `FeatureSet` with `Feature` instances built from catalogue tables or source-stats dictionaries; holds `ICoordinateTransformer` and `ISourceStatsProvider` only (set creation and visualiser binding live on the caller — see §6.3). Reads the listener from `FeatureSet.Listener` at attach time.
...
- `IFeatureSetCatalog`, `IFeatureFactory`, `ISelectionVisualiser`, `IActiveFeatureSetTypeProvider`, `IVisualiserBinder` — ST5-internal ports (not part of the cross-team contract). `IVisualiserBinder` is realised in the Unity ACL because the visualiser it constructs is a `MonoBehaviour`.
...
No ST4 → ST5 interface is consumed. `IActiveFeatureSetTypeProvider` is ST5-internal: both the producer (the source-list controller, ST5-owned per brief §6.5 "source-list statistics") and the consumer (`SelectionService`) sit inside ST5. `SelectionService.SelectAtCursor` uses that internal provider to prioritise the active type during spatial search; when `ActiveType` is `null` (no source-list panel open), `SelectionService` falls back to scanning every loaded set in `FeatureSetType`-declaration order.
